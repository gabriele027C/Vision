...
def test_resolve_candidate_direction_matches_screener():
    # FASE 2: solo trend EMA50 — RS non esclude
    df = make_ohlcv()
    last = float(df["close"].iloc[-1])
    e50 = _e50_last(df)
    ...
    # mid RS non esclude più se il trend è long
    expected = "long" if last > e50 else ("short" if last < e50 else None)
    assert resolve_candidate_direction(0.5, last, e50, True, True) == expected
    assert resolve_candidate_direction(0.5, e50, e50, True, True) is None
...
def test_diagnose_asset_blockers_when_not_eligible():
    # FASE 2: regime short non esclude più — eleggibilità da trend EMA50.
    # Asset in uptrend con RS alto → eligible; blockers possono essere vuoti o di setup.
    regime = {
        "mode": "short",
        "long_allowed": False,
        "short_allowed": True,
    ...
    assert d["watchlist_eligible"] is True
    assert d["direction"] == "long"


def test_diagnose_asset_not_eligible_without_trend():
    regime = {
        "mode": "long",
        "long_allowed": True,
        "short_allowed": True,
    ...
    # Serie piatta: last ≈ e50 → nessuna direzione
    df = make_ohlcv(daily_return=0.0)
    # Forza close == e50-ish: natural_direction None
    d = diagnose_asset("crypto", "FLAT", df, regime, rs_score=0.5)
    # Con return 0 la direzione può essere ambigua; almeno blockers o not eligible
    assert isinstance(d["watchlist_eligible"], bool)
